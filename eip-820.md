...
This standard keeps backwards compatibility with EIP165
...
Interfaces where the last 28 bits are `0` are considered EIP165 interfaces, and this registry
will forward the call to the contract to see if they implement that interface.

This contract will act also as an EIP165 cache in order to safe gas.
...
`sha3("ERC20Token")`
`sha3("ERC777Token")`
`sha3("ERC777TokensReceicver")`
`sha3("ERC777TokensSender")`

#### EIP165 compatible interfaces

Interfaces that the last 28bytes are 0, then this will be considered an EI165 interface.
...
This standard is backwards compatible with EIP165, as both methods can be implemented without conflicting one each other.
